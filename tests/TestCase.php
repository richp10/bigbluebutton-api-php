<?php
    ...
    class TestCase extends \PHPUnit\Framework\TestCase
    {
        ...
        protected function getBreakoutCreateMock($params)
        {
            ...
            return $createMeetingParams->setBreakout($params['isBreakout'])
        ...
        protected function getJoinMeetingMock($params)
        {
            ...
            return $joinMeetingParams->setUserId($params['userId'])
        ...
        public function assertEachGetterValueIsString($obj, $getters)
        {
            foreach ($getters as $getterName) {
                $this->assertIsString($obj->$getterName(),
                    'Got a ' . gettype($obj->$getterName()) . ' instead of a string for property -> ' . $getterName);
            }
        }
        ...
        {
            foreach ($getters as $getterName) {
                $this->assertIsInteger($obj->$getterName(),
                    'Got a ' . gettype($obj->$getterName()) . ' instead of an integer for property -> ' . $getterName);
            }
        }
        ...
        {
            foreach ($getters as $getterName) {
                $this->assertIsDouble($obj->$getterName(),
                    'Got a ' . gettype($obj->$getterName()) . ' instead of a double for property -> ' . $getterName);
            }
        }
        ...
        {
            foreach ($getters as $getterName) {
                $this->assertIsBoolean($obj->$getterName(),
                    'Got a ' . gettype($obj->$getterName()) . ' instead of a boolean for property -> ' . $getterName);
            }
        }
    }
